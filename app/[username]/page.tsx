...
export default async function PublicProfilePage({ params }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg-primary)', padding: '60px 24px 40px' }}>
      <div style={{ maxWidth: '560px', margin: '0 auto' }}>

        {/* Host Header */}
        <div style={{ textAlign: 'center', marginBottom: '48px' }}>
          <div style={{ width: '80px', height: '80px', borderRadius: '20px', margin: '0 auto 20px', overflow: 'hidden', background: 'linear-gradient(135deg, #6c5ce7, #a29bfe)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', flexShrink: 0 }}>
            {logoUrl ? (
              <Image
                src={logoUrl}
                alt={`${hostName} logo`}
          ...
          <h1 style={{ fontSize: '26px', fontWeight: 700, marginBottom: '8px' }}>{hostName}</h1>
          <p style={{ fontSize: '14px', color: 'var(--color-text-secondary)' }}>Choose a meeting type to get started</p>
        </div>

        {/* Booking Types */}
        {bookingTypes.length === 0 ? (
          <div className="glass-card" style={{ padding: '48px', textAlign: 'center' }}>
            <p style={{ color: 'var(--color-text-muted)', fontSize: '14px' }}>No booking types available at the moment.</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {bookingTypes.map((type: any) => (
              <Link key={type.id} href={`/${user.slug}/${type.slug}`} style={{ textDecoration: 'none' }}>
                <div className="glass-card" style={{ padding: '24px 28px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'transform 0.15s ease' }}>
                  <div>
                    <h3 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '6px', color: 'var(--color-text-primary)' }}>{type.title}</h3>
                    <div style={{ display: 'flex', gap: '12px', fontSize: '13px', color: 'var(--color-text-muted)' }}>
                      <span>⏱ {type.duration} min</span>
                      <span>{type.price && type.price > 0 ? `💰 ${type.price} ${type.currency}` : '✓ Free'}</span>
                    </div>
                  </div>
