...
/**
 * Groq Cloud model for structured extraction.
 * `llama-3.1-70b-versatile` was retired; `llama-3.3-70b-versatile` was the
 * replacement, but Groq has since moved it (and llama-3.1-8b-instant) behind
 * an Enterprise-only "Contact Sales" tier, no longer reachable with a
 * standard API key (confirmed via https://console.groq.com/docs/models).
 * Switched to openai/gpt-oss-120b — the closest capability match still on
 * Groq's standard self-serve, metered pricing.
 */
export const GROQ_MEETING_PARSER_MODEL = 'openai/gpt-oss-120b' as const;
...
/**
* Calls Groq (GPT-OSS 120B by default) to extract structured meeting fields from free-form text.
